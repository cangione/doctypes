...
<xsl:param name="footnote.number.symbols" select="''"/>
	...
	<xsl:param name="tmnotxt">
		...
		<xsl:choose>
			<xsl:when test="parent::tmno=tminfono">
				<xsl:value-of select="//paper.manual/paper.frnt//tminfono[1]/tmno | //paper.manual/volume/paper.frnt//tminfono[1]/tmno | //bdar/@pubno | //destruction_manual/paper.frnt//tminfono[1]/tmno"/>
			</xsl:when>
			<xsl:otherwise>
