...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="a.statement">
		<fo:block space-before.conditionality="discard" space-before.maximum="8pt" space-before.minimum="6pt" space-before.optimum="7pt" span="all" text-align="left">
			...
			<xsl:text>&#x2002;&#x2013;&#x2002;Approved for public release: distribution unlimited.</xsl:text>
		</fo:block>
	</xsl:template>
	<!-- As seen in Notices  -->
	...
	<xsl:template match="c.statement">
		<fo:block xsl:use-attribute-sets="notice-attrs">
			...
			<xsl:text>&#x2002;&#x2013;&#x2002;Distribution authorized to U.S. Government Agencies and their contractors only</xsl:text>
...
<xsl:template match="e.statement">
	...
	<xsl:template match="frgngvt">
			<xsl:text>; Foreign Government Information</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="softwaredoc">
			<xsl:text>; Software Documentation</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="crittech">
			<xsl:text>; Critical Technology</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="adminops">
			<xsl:text>; Administrative or Operational Use</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="specauth">
			<xsl:text>; Specific Authority</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	...
	<xsl:template match="testeval">
			<xsl:text>; Test and Evaluation</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="vulninfo">
			<xsl:text>; Vulnerability Information</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
		<xsl:template match="exportctrl">
			<xsl:text>; Export Controlled</xsl:text>
				<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
		<xsl:template match="opsec">
			<xsl:text>; Operations Security</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="cntrctperform">
			<xsl:text>; Contractor Performance Evaluation</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="premature">
			<xsl:text>; Premature Dissemination</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
	<xsl:template match="milsprt">
			<xsl:text>; Direct Military Support</xsl:text>
			<!--<xsl:value-of select="@distreason"/>-->
	</xsl:template>
