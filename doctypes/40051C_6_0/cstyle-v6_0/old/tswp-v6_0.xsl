...
<!-- LB CHANGED 10/29/2014
	...
	<xsl:template match="hookup">
		<fo:block span="all" xsl:use-attribute-sets="titlesub">HOOK-UP</fo:block>
		<xsl:apply-templates/>
	</xsl:template>
	<xsl:template match="disconnect">
		<fo:block span="all" xsl:use-attribute-sets="titlesub">DISCONNECT</fo:block>
		<xsl:apply-templates/>
	</xsl:template>
</xsl:stylesheet>
